...
Partial Class Part
    ...
    Protected Sub submitbtn_Click(sender As Object, e As EventArgs) Handles submitbtn.Click
        ...
        If Session("partID").ToString <> "" Then
            appcode.UpdatePart(Session("partID"), Session("part_type"), Trim(manufacturerdd.SelectedValue), Trim(partnumbertb.Text), descriptiontb.Text, altmfrdd.SelectedValue, altpntb.Text, oemmfrdd.SelectedValue, oempntb.Text, Trim(quantitytb.Text), uomtb.Text, pricetb.Text, "")
        Else
            Dim partID As Integer = appcode.InsertPart(Session("equipmentID"), Session("part_type"), Trim(manufacturerdd.SelectedValue), Trim(partnumbertb.Text), descriptiontb.Text, altmfrdd.SelectedValue, altpntb.Text, oemmfrdd.SelectedValue, oempntb.Text, Trim(quantitytb.Text), uomtb.Text, pricetb.Text, "")
        End If
    ...
    Protected Sub partnumbertb_TextChanged(sender As Object, e As EventArgs) Handles partnumbertb.TextChanged
        ...
        If reader.Read Then
            ...
            Dim alt_manufacturer As String = appcode.GetXRefManufacturer(reader.Item("manufacturer"), reader.Item("partnumber"))
            Dim alt_partnumber As String = appcode.GetXRefPartNumber(reader.Item("manufacturer"), reader.Item("partnumber"))
            If alt_manufacturer <> "" Then
                altmfrdd.SelectedValue = alt_manufacturer
                altpntb.Text = alt_partnumber
            End If
        End If
        reader.Close()
        conn.Close()
