<%@ Page Title="Active Quotes" Language="VB" MasterPageFile="Admin.master" AutoEventWireup="false" CodeFile="ActiveQuotes.aspx.vb" Inherits="EST_ActiveQuotes" %>
...
<asp:SqlDataSource ID="SqlQuotes" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT * FROM [t_quote] WHERE vendorID=@vendorID and complete=@complete and DATEADD(day,30,response_date) > getdate() ORDER BY [quoteID]">
    <SelectParameters>
        <asp:SessionParameter Name="vendorID" SessionField="vendorID" Type="Int32" />
        <asp:Parameter DefaultValue="True" Name="complete" />
    </SelectParameters>
</asp:SqlDataSource>
<asp:SqlDataSource ID="SqlQuotesByVUser" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT * FROM [v_quotesbyvuser] WHERE userID=@userID and complete=@complete and DATEADD(day,30,response_date) > getdate() ORDER BY [quoteID]">
    <SelectParameters>
