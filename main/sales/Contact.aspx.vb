...
Partial Class Contact
    ...
    Protected Sub savebtn_Click(sender As Object, e As EventArgs) Handles savebtn.Click
        Dim companyID As Integer
        companyID = Session("selected_companyID")
        If appcode.IsEmailAvailable(Trim(c_emailtb.Text), Session("selected_userID").ToString) = True Then
            If Session("selected_userID").ToString <> "0" Then
                'appcode.UpdateUser(Session("selected_userID"), c_nametb.Text, c_titletb.Text, c_phonetb.Text, c_faxtb.Text, c_emailtb.Text, c_passwordtb.Text, admincb.Checked)
                appcode.UpdateUser(Session("selected_userID"), c_nametb.Text, c_titletb.Text, c_phonetb.Text, c_faxtb.Text, c_emailtb.Text, c_passwordtb.Text, admincb.Checked, c_emailtb.Text)
            Else
                'Session("selected_userID") = appcode.InsertUser(c_nametb.Text, c_titletb.Text, c_phonetb.Text, c_faxtb.Text, c_emailtb.Text, c_passwordtb.Text, companyID, admincb.Checked)
                Session("selected_userID") = appcode.InsertUser(c_nametb.Text, c_titletb.Text, c_phonetb.Text, c_faxtb.Text, c_emailtb.Text, c_passwordtb.Text, companyID, admincb.Checked, c_emailtb.Text)
